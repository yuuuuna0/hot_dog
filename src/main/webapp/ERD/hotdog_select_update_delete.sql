...
--5. orders, order_item, payment, product, userinfo JOIN
select * from orders o join order_item oi on o.o_no=oi.o_no join payment pm on o.pm_no=pm.pm_no join product p on oi.p_no=p.p_no join userinfo u on o.u_id=u.u_id;

select * from orders o join order_item oi on o.o_no=oi.o_no join payment pm on o.pm_no=pm.pm_no join product p on oi.p_no=p.p_no join userinfo u on o.u_id=u.u_id where o.u_id='sy0';

select * from orders o join order_item oi on o.o_no=oi.o_no join payment pm on o.pm_no=pm.pm_no join product p on oi.p_no=p.p_no join userinfo u on o.u_id=u.u_id where o.o_no=2;
