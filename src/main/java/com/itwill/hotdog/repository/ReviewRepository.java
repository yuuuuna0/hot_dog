...
public int createReply(Review review) throws Exception{
	...
	try {
		...
		Review temp = this.findReview(review.getR_no());
		...
		pstmt.setInt(2, review.getR_grade()); //평점
		pstmt.setInt(3, temp.getR_groupNo()); // group no
		pstmt.setInt(4, review.getR_step()+1); // step
		pstmt.setInt(5, review.getR_depth()+1);// depth
		pstmt.setString(6, review.getU_id()); // 작성자
		pstmt.setInt(7, review.getP_no()); //상품번호
