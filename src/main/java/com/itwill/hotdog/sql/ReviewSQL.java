...
public class ReviewSQL {
	...
	public static final String REVIEW_SELECT_BY_P_NO = "select * from review where p_no=? order by r_groupno desc, r_step asc";
	public static final String REVIEW_SELECT_BY_PNO = "SELECT * FROM (SELECT rownum idx, s.* FROM ( SELECT * from review ORDER BY r_groupno DESC, r_step ASC ) s where s.p_no = ?) WHERE idx >= ? AND idx <= ?";
	public static final String REVIEW_SELECT_BY_GNO = "SELECT * FROM (SELECT rownum idx, s.* FROM ( SELECT * from review ORDER BY r_groupno DESC, r_step ASC ) s where s.r_groupno = ?) WHERE idx >= ? AND idx <= ?";
	...
	public static final String REVIEW_COUNT_PNO = "select count(*) from review where p_no=?";
	}
