...
public class OrdersSQL_sy {
	...
	public static final String ORDERS_PAYMENT_USERINFO_JOIN_SELECT_BY_U_ID = "select * from orders o join payment pm on o.pm_no=pm.pm_no join userinfo u on o.u_id=u.u_id where o.u_id=? order by o.o_date desc";
	public static final String ORDERS_PAYMENT_USERINFO_ORDERITEM_PRODUCT_JOIN_SELECT_BY_O_NO
	= "select * from orders o join payment pm on o.pm_no=pm.pm_no join userinfo u on o.u_id=u.u_id join order_item oi on o.o_no=oi.o_no join product p on oi.p_no=p.p_no where o.o_no=? order by o.o_date desc";
}